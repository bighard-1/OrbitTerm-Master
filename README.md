...
控制台使用独立管理员登录页，登录后可完成用户治理、邀请码、策略、审计与加密迁移。它复用 `/api/v1/admin/...` 接口和 JWT 权限体系，管理令牌仅保存在当前浏览器标签页。

普通用户注册采用三重门槛：管理员邀请码、允许的邮箱后缀、至少 12 位且包含大小写字母/数字/特殊字符的密码。主流邮箱后缀已有安全默认值，管理员可在策略页维护自定义后缀。
...
- `RegistrationInvite`
  - 数据库只保存邀请码 SHA-256 摘要和前缀，不保存可重新读取的明文邀请码
  - 支持有效期、使用次数、撤销和管理员审计
...
- `POST /api/v1/admin/system/migration-bundle/export`
- `POST /api/v1/admin/system/migration-bundle/restore`
- `GET/POST /api/v1/admin/registration-invites`
- `POST /api/v1/admin/registration-invites/:id/revoke`
...
curl -X POST http://localhost:8080/api/v1/auth/register \
  ...
  -d '{"username":"alice@gmail.com","password":"StrongPass123!","invite_code":"管理员生成的一次性邀请码"}'
```
...
curl -X POST http://localhost:8080/api/v1/auth/login \
  -H 'Content-Type: application/json' \
  -d '{"username":"alice@gmail.com","password":"StrongPass123!"}'
```
...
启动时会自动执行用户、邀请码、密文配置、同步修订、设备确认水位、审计日志与系统策略的兼容迁移。

所有服务端时间戳和 PostgreSQL 会话统一使用 UTC；管理端仅在显示时转换为浏览器本地时区。部署主机可使用本地时区，但容器与数据库建议固定 `TZ=UTC`，避免夏令时和跨区域迁移造成歧义。
...
| 能力 | 解决的问题 | 当前项目状态 |
| --- | --- | --- |
| 数据备份 | 误删除、数据库损坏、升级失败后恢复数据 | 支持管理端加密全量迁移包；生产灾备仍需经过演练的 PostgreSQL 备份 |
