...
func TestAdminConsoleIncludesGovernanceEntrypoints(t *testing.T) {
	...
	appText := string(app)
	for _, marker := range []string{"sessionStorage", "reset-password", "soft-delete", "backup-readiness", "system/diagnostics", "batchUserAction", "exportAudit", "createManagedUser", "/role"} {
